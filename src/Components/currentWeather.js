...
export const getWeatherForLocationProvided = () => {
    ...
    axios.request(weatherAPICall).then(function (response) {
        const responseLocationInformation = response.data.location
        const responseCurrentWeatherInformation = response.data.current
        console.log(responseLocationInformation)
        console.log(responseCurrentWeatherInformation)
        document.getElementById('currentWeatherResponse').innerHTML = appendCurrentWeatherResponse(responseLocationInformation, responseCurrentWeatherInformation)
    }).catch(function (error) {
        console.error(error);
    });
}

export const appendCurrentWeatherResponse = (responseLocationInformation, responseCurrentWeatherInformation) => {
    const currentTemp = responseCurrentWeatherInformation.temp_c
    const currentWindSpeed = responseCurrentWeatherInformation.wind_mph
    const currentHumidity = responseCurrentWeatherInformation.humidity
    const currentVisibility = responseCurrentWeatherInformation.vis_miles
    const currentWindDirection = responseCurrentWeatherInformation.wind_dir
    const weatherDescription = responseCurrentWeatherInformation.condition.text
    const currentUV = responseCurrentWeatherInformation.uv

    const currentLocation = responseLocationInformation.name
    const currentLocationLongitude = responseLocationInformation.lon
    const currentLocationLatitude = responseLocationInformation.lat

    return getLocationSvg() + '<p>' + currentLocation + '</p>'
        + getWeatherSvg(weatherDescription) + '<p>' + (currentTemp + "'c" ) + '</p>' +
        '<p>' + 'Current Longitude: ' + currentLocationLongitude + '</p>' +
        '<p>' + 'Current Latitude: ' + currentLocationLatitude + '</p>' +
        '<p>' + 'Current Wind Speed: ' + currentWindSpeed + 'mph' + '</p>' +
        '<p>' + 'Current Humidity: ' + currentHumidity + '%' + '</p>' +
        '<p>' + 'Current Visibility: ' + currentVisibility + 'miles' + '</p>' +
        '<p>' + 'Current Wind Direction: ' + currentWindDirection + '</p>' +
        '<p>' + 'Current UV Index: ' + currentUV + '</p>'
}

export const getWeatherSvg = weatherDescription => {
    if ( weatherDescription === 'Sunny' ) return getSunSvg()
    if ( weatherDescription === 'Overcast' ) return getOvercastSvg()
    return 'Current Temperature Is: '
}

export const getOvercastSvg = () => {
    return '<?xml version="1.0" encoding="iso-8859-1"?>\n' +
        '<!-- Generator: Adobe Illustrator 19.0.0, SVG Export Plug-In . SVG Version: 6.00 Build 0)  -->\n' +
        '<svg version="1.1" id="OvercastSvg" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"\n' +
        '\t viewBox="0 0 247.536 247.536" style="enable-background:new 0 0 247.536 247.536;" xml:space="preserve">\n' + '<g>\n' +
...
export const getLocationSvg = () => {
    return '<?xml version="1.0" encoding="iso-8859-1"?>\n' +
        '<!-- Generator: Adobe Illustrator 18.0.0, SVG Export Plug-In . SVG Version: 6.00 Build 0)  -->\n' +
        '<!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">\n' +
        '<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="75" height="75"\n' +
        '\t viewBox="0 0 264.018 264.018" style="enable-background:new 0 0 264.018 264.018;" xml:space="preserve" class="mt-1">\n' +
        '<g>\n' +
...
export const getSunSvg = () => {
    return '<?xml version="1.0" encoding="iso-8859-1"?>\n' +
        '<!-- Generator: Adobe Illustrator 18.0.0, SVG Export Plug-In . SVG Version: 6.00 Build 0)  -->\n' +
        '<!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">\n' +
        '<svg version="1.1" id="SunSvg" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="75" height="75"\n' +
        '\t viewBox="0 0 187.322 187.322" style="enable-background:new 0 0 187.322 187.322;" xml:space="preserve" class="mt-1">\n' +
        '<g>\n' +
